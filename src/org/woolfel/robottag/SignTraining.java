...
public class SignTraining {
	...
	protected static int epochs = 6;

	public static final Random randNumGen = new Random();
	private static Logger log = LoggerFactory.getLogger(SignTraining.class);
		...
		try {
			...
			for (int i=0; i < 1000; i++) {
				...
				for (int e = 0; e < epochs; e++) {
					...
					log.info(eval.stats());
					if (eval.accuracy() > maxAccurate) {
						maxAccurate = eval.accuracy();
					}
					System.out.println("!--------------------------------- Best accuracy so far: " + maxAccurate);
					if (eval.accuracy() > 0.98) {
						try {
							 ModelSerializer.writeModel(model, new File("./data/sign-gm-" + i + ".model"), true);
